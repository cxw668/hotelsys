<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.role" placeholder="职位" clearable class="filter-item" style="width: 130px">
...
export default {
  ...
  methods: {
    ...mapActions('hotel', ['getEmployees', 'addEmployee', 'updateEmployee', 'deleteEmployee']),
    async getList() {
    ...
    filterData() {
      ...
      if (this.listQuery.position) {
        items = items.filter(item => item.position === this.listQuery.position)
      }
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.addEmployee(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          this.updateEmployee(tempData).then(() => {
            this.dialogFormVisible = false
            this.$notify({
              title: '成功',
              message: '更新成功',
    ...
    handleDelete(row, index) {
      this.$confirm('确认删除?', '提示', {
        ...
        this.deleteEmployee(row.id).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
